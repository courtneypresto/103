...
int Network::read_friends(char* fname){
    ...
    int network_size;                      //first thing in the file is network size
    ifile >> network_size;
    ...
    for (int i = 0; i < network_size; i++){ 
        ...
        indexy.push_back(any_user);             //adds all the users to the index
        }
    return 0;
}
...
void Network::add_user (User bob){
    indexy.push_back(bob);                      // adds a new user to the index
    }    
...
int Network::write_friend (char* fname){
    ...
    if (ofile.fail()){       // quit id we can't successfully make an output file
        return -1;
    }    
...
ofile << indexy.size() << endl;
    for (int i =0; i < (int)indexy.size(); i++){ 
         User lola = indexy[i];             // creates user objects to print in output file
         ofile <<  lola.what_id() << "\n\t" << lola.what_first_name() << " " << lola.what_last_name() << "\n\t" << lola.what_year() << "\n\t" <<
        lola.what_zip() << "\n\t";
...
int Network::get_id (string fname, string lname){
    for (int i=0; i < (int)indexy.size(); i++){
        ...
        if ((bob.what_first_name() == fname) && (bob.what_last_name() ==lname)){
...
vector<int> Network::shortest_path(int from, int to){
    ...
    queue.push_back(from);                 // put from as the first person in q                           
    vector<int> path;                           // path 
    vector<int> path1;                          // path 1
    indexy[from].change_pred(-1);               // from's pred is -1
    indexy[from].is_not_eli();
    //int pred=100;                               // pred is 100 ... to be used later
    int possible =-10;                          // check variable

    while (head != (int) queue.size()){         // while head is not at the end

        vector <int> some_friends =indexy[queue[head]].what_friends(); // creates a vector for whatever's 
...
vector<vector<int> > Network::groups(){
    ...
    for (int i=0; i< (int) indexy.size(); i++){     //resets the bool variable
        indexy[i].reset_visit();
    }

    while (a_check < (int) indexy.size()){ // until we visit every user in the indexy
        a_check = 0;                    
        for (int i=0; i < (int)indexy.size(); i++){
            if (indexy[i].get_visit() == false){      //if we haven't visited wayne b4
            big_vector.push_back(this->one_group(i)); // call one_group to make a group starting with him
             }
        }
           
            for (int i=0; i < (int) indexy.size(); i++){ // first, see where we are with the check 
                if (indexy[i].get_visit() == true){
                    a_check++;                           // how many people in the index                 
...
vector<int> Network::one_group(int a){
      vector<int> queue1;
      vector <int> test = indexy[a].what_friends(); //vector for wayne's friends
    ...
    while (head1 !=(int) queue1.size()){
        ...
        while (i < (int) some_friends1.size()){
                if (indexy[some_friends1[i]].get_visit() == false){  // if not visited
...
vector<int> Network::suggest_friends(int who, int& score){
   //cout << "who is "<< who << endl;
    vector <int> testA = indexy[who].what_friends();       // vector for this guy's friends
    vector <int> mutual;                                // vector to hold all mutual friends     
    ...
    for (int i = 0; i < (int) testA.size(); i++){ //set friends as ineligable
        ...
        //cout << indexy[testA[i]].what_first_name() << " is not eligable " << endl;
    }
      indexy[who].not_pos();       
    //cout << endl;      
                    // Deals with the score //
    for (int i=0; i< (int) testA.size(); i++){                  // for all of who's friends
        vector <int> paul_f = indexy[testA[i]].what_friends(); // vector for (who's friends friend)
        //cout << "we're on " << indexy[testA[i]].what_first_name()<< endl;
        /*for (int k=0; k < (int) paul_f.size(); k++){
            cout << "paul_f at " <<i << " is " << indexy[paul_f[k]].what_first_name() << endl;
            }*/
        //cout <<" \n\n" << indexy[testA[i]].what_first_name() << "'s friend is: " << indexy[paul_f
        //[i]].what_first_name() << endl;
        for(int j=0; j < (int) paul_f.size(); j++){           // for all of who's friends friends
            if (indexy[paul_f[j]].return_pos_mut() == true){          // if they are eligable  
                 indexy[paul_f[j]].add_score();                     // score goes up
                 //cout << "a potential is: "<<  indexy[paul_f[i]].what_first_name() << endl;
                if (indexy[paul_f[j]].is_on_mutual() == false){ // if they aren't in the mutual vect
                 mutual.push_back(indexy[paul_f[j]].what_id()); // put in vetor
                 indexy[paul_f[j]].change_mutual();             // no longer mutal
                }
            }
        }
    }
...
/*for (int i=0; i < (int) mutual.size(); i++){
    cout << "Mutual number: " << i << " is "<<  mutual[i];
 }*/
...
vector <int> suggest;
   ...
   for (int i=0; i < (int) mutual.size(); i++){   // goes through the list of mutual friends
        User phi = indexy[mutual[i]];
        if (phi.what_score() > scorey){
            scorey = phi.what_score();               // finds what the highest score is
        }
    }
    ...
    for (int i=0; i < (int) mutual.size(); i++){    // goes through the list of mutual friends 
        if (indexy[mutual[i]].what_score() == scorey){
            suggest.push_back(indexy[mutual[i]].what_id());     // creates a vector of everyone with the 
                                                                //highest score
        }
    }
